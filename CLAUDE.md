...
**New here?** [docs/ONBOARDING.md](docs/ONBOARDING.md) §1 is a runnable setup path: `bash setup/setup_conda.sh` -> `make bootstrap` -> `make test-fast` (**546 passed, 3 skipped, 2 deselected** as of 2026-09-24; ~3m30s in CI, well under a minute on a warm dev box; needs no data). Data custody and the `gh release download` pull are in [DATA.md](DATA.md).
