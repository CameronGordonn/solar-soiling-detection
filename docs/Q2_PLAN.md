...
- **Stage 2 (risk):** ⚠️ **Superseded 2026-09-22 — the Stage 2 gates were measured on leaking folds.** Both AUC folds hold out one axis and not the other (88.7% of rows keep their station in training); the honest joint out-of-station-and-year AUC is **0.622** [0.571, 0.670], not 0.710, and lat/lon alone score 0.644. Regional limit corrected to **0.655** pooled / **0.527** worst region. Calibration is unaffected. See [CANONICAL_NUMBERS.md](CANONICAL_NUMBERS.md) and `paper/paper.tex` §4.4. The line below is the pre-correction record, kept because the registry flag it describes is still in place.
  **GA-ready within its training geography** — all three gates recorded as clear (spatial-CV AUC **0.712** re-measured, pooled out-of-year AUC 0.710, calibration retained). **GA in the registry as of 2026-09-02** — `run_optionb` is `beta: false`, closing the last flag that was waiting on a person rather than on evidence. GA does not retire the regional limit below; a caller quoting a number outside the training geography still has to read `known_limitations`. **The 0.728 in older copies is `run_optionb`'s stored figure and does not reproduce** under the current `model.yaml`; quote 0.712 and describe the model as *at* its gate. **Known hard limit (measured 2026-08-30):** pooled out-of-region AUC **0.677**, worst region **0.548** — the model does not transfer to a region unlike its training geography, and "model work is done" means *on this label set*, not in general. See [CANONICAL_NUMBERS.md](CANONICAL_NUMBERS.md) and [PVDAQ_LANE_HANDOFF_20260831.md](PVDAQ_LANE_HANDOFF_20260831.md).
...
> ⚠️ **Superseded 2026-09-22.** The replacement gate described below is itself retired: its fold
> holds out the year but not the station (88.7% of rows keep their station in training), so the
> 0.710 it reports is a leaking figure. Honest joint-fold AUC: **0.622** [0.571, 0.670]. See
